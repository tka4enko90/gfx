...
<form id="affwp-register-form" class="affwp-form" action="" method="post">
    ...
    <fieldset>
        ...
        <p>
            <label for="affwp-user-name"><?php _e('Full Name', 'gfx'); ?><span>*</span></label>
            <input id="affwp-user-name" type="text" name="affwp_user_name" value="<?php if (!empty($user_name)) {
                echo $user_name;
            } ?>"
                   title="<?php esc_attr_e('Your Name', 'gfx'); ?>" <?php echo affwp_required_field_attr('your_name'); ?> />
        </p>

        <p>
            <label for="affwp-user-login"><?php _e('Username', 'gfx'); ?><span>*</span></label>
            <input id="affwp-user-login" required="required" type="text" name="affwp_user_login" maxlength="60"
                   value="<?php if (!empty($user_login)) {
                       echo $user_login;
                   } ?>" title="<?php esc_attr_e('Username', 'gfx'); ?>"<?php echo $disabled; ?> />
        </p>

        <p>
            <label for="affwp-user-email"><?php _e('Account Email', 'gfx'); ?><span>*</span></label>
            <input id="affwp-user-email" required="required" type="email" name="affwp_user_email"
                   value="<?php if (!empty($user_email)) {
                       echo $user_email;
                   } ?>" title="<?php esc_attr_e('Email Address', 'gfx'); ?>"<?php echo $disabled; ?> />
        </p>
        ...
        <p>
            ...
            <input id="affwp-payment-email" type="email" name="affwp_payment_email"
                   ...
                   title="<?php esc_attr_e('Payment Email Address', 'gfx'); ?>"<?php echo affwp_required_field_attr('payment_email'); ?> />
        </p>

        <p>
            <label for="affwp-user-url"><?php _e('Website URL', 'gfx'); ?></label>
            <input id="affwp-user-url" type="text" name="affwp_user_url" value="<?php if (!empty($url)) {
                echo $url;
            } ?>"
                   title="<?php esc_attr_e('Website URL', 'gfx'); ?>" <?php echo affwp_required_field_attr('website_url'); ?> />
        </p>

        <p>
            <label for="affwp-promotion-method"><?php _e('How will you promote us?', 'gfx'); ?></label>
            <textarea id="affwp-promotion-method" name="affwp_promotion_method" rows="5"
                      cols="30"<?php echo affwp_required_field_attr('promotion_method'); ?>><?php if (!empty($method)) {
        ...
        <?php if (!is_user_logged_in() && isset($required_registration_fields['password'])) : ?>
            ...
            <p>
                <label for="affwp-user-pass"><?php _e('Password', 'gfx'); ?></label>
                <input id="affwp-user-pass" required="required" class="password" type="password"
                       name="affwp_user_pass"/>
            </p>

            <p>
                <label for="affwp-user-pass2"><?php _e('Confirm Password', 'gfx'); ?></label>
                <input id="affwp-user-pass2" required="required" class="password" type="password"
        ...
        $terms_of_use_label_text = get_field('terms_of_use_label_text', 'option');

        if (!empty($terms_of_use) && !empty($terms_of_use_label_text)) : ?>
            <div class="terms-holder">
                <label class="affwp-tos" for="affwp-tos">
                    <input id="affwp-tos" required="required" type="checkbox" name="affwp_tos"/>

                    <?php echo $terms_of_use_label_text; ?>
                </label>
            </div>
        <?php endif; ?>
        ...
        <p>
            ...
            <input type="hidden" name="affwp_register_nonce"
                   value="<?php echo wp_create_nonce('affwp-register-nonce'); ?>"/>
            <input type="hidden" name="affwp_action" value="affiliate_register"/>
            <input class="primary-button" type="submit" value="<?php esc_attr_e('Register', 'gfx'); ?>"/>
        </p>
