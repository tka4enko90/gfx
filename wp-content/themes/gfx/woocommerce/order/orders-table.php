...
<?php if (isset($items) && isset($offset) && isset($items_per_page)) : ?>
    <table class="orders-ajax-table woocommerce-orders-table woocommerce-MyAccount-orders shop_table shop_table_responsive my_account_orders account-orders-table">
        <thead>
        <tr>
        ...
        <?php $sliced_customer_orders = array_slice($items, $offset, $items_per_page); ?>
        <tbody>
            <?php foreach ($sliced_customer_orders as $customer_order) {
                if(is_string($customer_order)) :
                    $order = wc_get_order(json_decode($customer_order)->id);
                else :
                            ...
                            <?php elseif ('order-actions' === $column_id) : ?>
                                ...
                                if (!empty($actions)) {
                                    foreach ($actions as $key => $action) { // phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited
                                        $btn_classes = 'small ';
                                        if ($key == 'pay') :
                                            $btn_classes .= ' primary-button blue ';
                                        elseif ($key == 'cancel') :
                                            $btn_classes .= ' secondary-button ';
                                        else :
                                            $btn_classes .= ' primary-button ';
                                        endif;

                                        echo '<a href="' . esc_url($action['url']) . '"
                                                class="' . $btn_classes . sanitize_html_class($key) . '">' . esc_html($action['name']) . '</a>';
                                    }
                                }
                                ?>
                            <?php endif; ?>
