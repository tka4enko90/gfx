...
if ( isset( $form_data ) ) :
	foreach ( $form_data as $key => $value ) :
		...
		// sort-by
		if ( 'sort-by' === $key ) :
			if ( 'date-new-old' === $value ) :
				$args['orderby'] = 'date';
				$args['order']   = 'DESC';
			endif;

			if ( 'date-old-new' === $value ) :
				$args['orderby'] = 'date';
				$args['order']   = 'ASC';
			endif;

			if ( 'alphabetically-a-z' === $value ) :
				$args['orderby'] = 'title';
				$args['order']   = 'ASC';
			endif;

			if ( 'alphabetically-z-a' === $value ) :
				$args['orderby'] = 'title';
				$args['order']   = 'DESC';
			endif;

			if ( 'price-high-to-low' === $value ) :
				$args['orderby']  = 'meta_value_num';
				$args['meta_key'] = '_price';
				$args['order']    = 'DESC';
			endif;

			if ( 'price-low-to-high' === $value ) :
				$args['orderby']  = 'meta_value_num';
				$args['meta_key'] = '_price';
				$args['order']    = 'ASC';
			endif;
...
if ( $products->have_posts() ) :
	...
	if ( ! empty( $feature_product_ids ) && ! array_key_exists( 'orderby', $args ) ) {
		foreach ( $products->posts as $k => $post ) {
			$post_id = $post->ID;
			if ( in_array( $post_id, $feature_product_ids, true ) && 0 !== $k ) {
				$temp = $post;
