 ...
 * @since 2.1.9.210525
 */
class Freemius_Plugin_Pricing_Shortcode extends Base_Shortcode {
	/**
	 * The shortcode ID, which will be passed to the add_shortcode() function.
	 *
	 * @var string
	 */
	protected static $shortcode = 'aelia_freemius_plugin_pricing';
	...
	public function render($args): string {
		...
		try {
			...
			if(empty($args['plugin_slug'])) {
				...
				$message = implode(' ', [
					__('Freemius Plugin Pricing Shortcode', Definitions::TEXT_DOMAIN),
			...
			if(!Freemius_Plugin_Integration::is_plugin_license_activated($args['plugin_slug'])) {
				// If something goes wrong, return the error message
				$message = implode(' ', [
					__('Freemius Plugin Pricing Shortcode', Definitions::TEXT_DOMAIN),
			...
			return '<div class="freemius_plugin_pricing">' . wp_kses_post($e->getMessage()) . '</div>';
		}

		ob_start();
		?>
		<div class="freemius_plugin_pricing"><?php
			// TODO Find a way to render the pricing page correctly
			// @link https://github.com/Freemius/wordpress-sdk/issues/498
			// @link https://github.com/Freemius/wordpress-sdk/issues/499
