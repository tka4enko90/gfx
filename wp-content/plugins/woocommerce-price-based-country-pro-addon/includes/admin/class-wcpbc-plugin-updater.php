	...
	class WCPBC_Plugin_Updater {
		...
		public function check_update( $_transient_data ) {
			...
			if ( is_wp_error( $data ) ) {
				...
				// Update the properties with the response.
				foreach ( $update as $property => $value ) {
					if ( ! empty( $data->{$property} ) ) {
						$update[ $property ] = in_array( $property, array( 'icons', 'banners' ), true ) ? (array) $data->{$property} : $data->{$property};
					}
				}
				...
				if ( version_compare( $this->version, $data->new_version, '<' ) ) {
					$update->package = add_query_arg( 'token', $this->api_data['api_key'], $data->package );

					$_transient_data->response[ $this->name ] = $update;
				} else {
					// Adding the item to the "no_update" property is required for the enable/disable auto-updates links.
					$_transient_data->no_update[ $this->name ] = (object) $update;
				}
