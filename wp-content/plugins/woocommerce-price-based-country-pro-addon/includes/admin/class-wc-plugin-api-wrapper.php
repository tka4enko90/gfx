		...
		private static function api_request( $action, $params, $assoc = false ) {
			...
			$response = wp_remote_post(
				...
				array(
					...
					'cookies'     => array(),
					'sslverify'   => false,
					'user-agent'  => 'WordPress/' . get_bloginfo( 'version' ) . '; ' . get_bloginfo( 'url' ) . '; ' . site_url(),
				)
			);
			...
			if ( is_wp_error( $response ) ) {
				...
				if ( 200 === absint( $response['response']['code'] ) ) {
					...
					if ( empty( $data ) ) {
						$logger->add( 'wc_price_based_country', 'Error on the request to the license API: ' . $response['response']['code'] . ' - ' . wp_strip_all_tags( $response['body'] ) );
					}
				} else {
					// Error.
